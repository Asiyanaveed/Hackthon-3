...
export default function Hero() {
  interface HeroData {
    heroImages: (string | StaticImport)[];
    heroHeading: string;
    heroPara: string;
    heroBtnText: string;
  }
  ...
  const { heroImages, heroHeading, heroPara, heroBtnText } = heroData
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mb-[84px]">
      ...
      <div className="w-full h-[700px] mb-16 overflow-hidden relative">
        ...
        >
          {heroImages.map((image: string | StaticImport, index: Key ) => (
      ...
      <div className="max-w-4xl mx-auto text-center space-y-6 px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-base text-gray-600 max-w-xl mx-auto">{heroPara}</p>

        {/* Buttons */}
        <div className="flex items-center justify-center gap-3 pt-4">
          <Button variant="default" className="rounded-full px-6 py-2 text-sm sm:text-base">
            Notify Me
          </Button>
          <Button variant="default" className="rounded-full px-6 py-2 text-sm sm:text-base">
            Shop {heroBtnText}
